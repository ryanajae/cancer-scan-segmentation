...
def printMenu(options_list):
    ...
    while optionLoop:
        ...
        elif int(response) > numOptions or int(response) < 0:
            print("Scan does not exist. Enter a number in range.")
        else: 
            responseInt = int(response) - 1 # Zero-indexed list
            if responseInt == -1: responseInt = 0 # 0-1 = -1, which is end of list
            choice = options_list[responseInt]
            print (choice)
